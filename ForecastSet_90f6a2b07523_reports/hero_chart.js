(() => {
    ...
    function renderHeroChart(payload) {
        ...
        chart
            ...
            .data(milestoneData)
            .join("line")
            .attr("class", "hero-milestone-guide")
            .attr("x1", (item) => xScale(item.__date))
            .attr("x2", innerWidth)
            .attr("y1", (item) => milestoneYScale(item.Amount))
            .attr("y2", (item) => milestoneYScale(item.Amount))
            .attr("stroke", options.milestone_color ?? "#a05a9c")
            .attr("stroke-width", 1)
        ...
        const milestonePoints = chart
            ...
            .on("mouseenter", (event, item) => {
                ...
                item.Milestones.forEach((milestone) => {
                    ...
                    rows.push([
                        "",
                        formatCurrency(Number(milestone.balance)),
                    ]);
                });
