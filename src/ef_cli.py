    ...
    for file_name in os.listdir(target_directory):
        if file_name.startswith("ForecastSet") and file_name.endswith(".json"):
            ...
            try:
                ...
                new_row_df = pd.DataFrame(
                    [
                        "file",
                        "Set",
                        S.base_forecast.unique_id,
                        len(S.forecast_name_to_budget_item_set__dict),
                        datetime.datetime.strptime(
                            S.base_forecast.start_date_YYYYMMDD, "%Y%m%d"
                        ).strftime("%Y-%m-%d"),
                        datetime.datetime.strptime(
                            S.base_forecast.end_date_YYYYMMDD, "%Y%m%d"
                        ).strftime("%Y-%m-%d"),
                        0,
                        S.base_forecast.start_ts,
                        0,
            ...
            try:
                ...
                satisfice_failed_ind = (
                    not E.forecast_df.tail(1).Date.iat[0] == E.end_date_YYYYMMDD
                )  # todo not sure if this is correct
                forecast_is_done_ind = E.forecast_df is not None
...
def main(args, loglevel):
    ...
    if args.source == "database" or args.source == "both":
        ...
        connection_test = pd.read_sql_query(
            "select 'connection successful'", con=engine
        )
    ...
    if len(args.action) == 1:
        if args.action[0] == "list" and args.source == "file":
            forecast_file_details = scrape_dir_for_forecast_details(
            ...
            forecast_database_details = pd.read_sql_query(
                ...
                CONCAT('prod."""
                + args.username
                + """_forecast_',deets.forecast_id) as resource_name
                from prod.virtuoso_user_staged_forecast_details deets
                LEFT JOIN prod."""
                + args.username
                + """_forecast_run_metadata meta
            ...
            for index, row in forecast_database_details.iterrows():
                ...
                if row.type == "F":
                    try:
                        ...
                        table_existence_ind = pd.read_sql_query(
                            table_existence_q, con=engine
                        ).iat[0, 0]
            ...
            forecast_database_details = pd.read_sql_query(
                ...
                LEFT JOIN prod."""
                + args.username
                + """_staged_forecast_details deets
            ...
            both_forecast_details = pd.concat(
                [forecast_file_details, forecast_database_details]
            )
        ...
        if args.action[0] == "inspect":
            ...
            elif args.filename.startswith("Forecast") and args.filename.endswith(
                ".json"
            ):
                try:
    ...
    if len(args.action) == 2:
        if (
            ...
            S = ForecastSet.initialize_from_json_file(
                args.filename
            )  # let this throw an exception if needed
            # print('BEFORE')
            # print(S)
            ...
            E.forecast_df.to_csv(
                args.working_directory + "/Forecast_" + str(E.unique_id) + ".csv",
                index=False,
            )
            ...
            try:
                S = ForecastSet.initialize_forecast_set_from_database(
                    set_id=args.id,
                    username=args.username,
                    database_hostname=args.database_hostname,
                    database_name=args.database_name,
            ...
            for unique_id, E in S.initialized_forecasts.items():
                E.writeToJSONFile(args.working_directory)
                E.forecast_df.to_csv(
                    args.working_directory + "Forecast_" + E.unique_id + ".csv",
                    index=False,
                )
                F.generateHTMLReport(E)
            if not forecast_set_found:
            ...
            temporary_memo_rule_set_table_name = (
                "prod.ef_memo_rule_set_" + args.username + "_temporary"
            )
            temporary_account_milestone_table_name = (
                "prod.ef_account_milestones_" + args.username + "_temporary"
            )
            temporary_memo_milestone_table_name = (
                "prod.ef_memo_milestones_" + args.username + "_temporary"
            )
            temporary_composite_milestone_table_name = (
                "prod.ef_composite_milestones_" + args.username + "_temporary"
            )

            forecast_set_def_name = (
                "prod." + args.username + "_forecast_set_definitions"
            )
            ...
            account_milestone_select_q = (
                "select * from " + temporary_account_milestone_table_name
            )
            memo_milestone_select_q = (
                "select * from " + temporary_memo_milestone_table_name
            )
            composite_milestone_select_q = (
                "select * from " + temporary_composite_milestone_table_name
            )
            ...
            set_def_q = (
                """select '' as forecast_set_id, '' as forecast_set_name, '' as forecast_id, '"""
                + args.label
                + """' as forecast_name, '' as start_date, '' as end_date, '' as insert_ts """
            )
            ...
            budget_item_post_run_category_select_q = (
                "select * from prod."
                + args.username
                + "_budget_item_post_run_category where forecast_id = 'FORECAST_NOT_YET_RUN'"
            )

            # If no forecast, this query will fail downstream
            forecast_select_q = "Nonsense query bc forecast does not exist on db yet"

            E = ExpenseForecast.initialize_from_database_with_select(
                args.start_date,
                args.end_date,
            ...
            M_insert_q = (
                ...
                + " Select '"
                + E.unique_id
                + "', memo_regex, account_from, account_to, priority from "
                + temporary_memo_rule_set_table_name
            )
            cursor.execute(M_insert_q)

            forecast_set_delete_q = (
                "DELETE FROM "
                + forecast_set_def_name
                + " where forecast_id = '"
                + E.unique_id
            ...
            # 	end_date date,
            # 	insert_ts timestamp
            forecast_set_insert_q = (
                "INSERT INTO "
                + forecast_set_def_name
                + " select '' as forecast_set_id, '' as forecast_set_name, "
            )
            forecast_set_insert_q += (
                "'"
                + E.unique_id
            ...
            account_milestone_table_name = "prod.ef_account_milestones_" + args.username
            memo_milestone_table_name = "prod.ef_memo_milestones_" + args.username
            composite_milestone_table_name = (
                "prod.ef_composite_milestones_" + args.username
            )
            ...
            # forecast_set_id, forecast_set_name, forecast_id, forecast_name, start_date, end_date, insert_ts
            set_def_q = """
               select '' as forecast_set_id, '' as forecast_set_name, '' as forecast_id, '' as forecast_name, '' as start_date, '' as end_date, '' as insert_ts
               """
            ...
            base_E = ExpenseForecast.initialize_from_database_with_select(
                args.start_date,
                args.end_date,
                account_set_select_q=account_set_select_q,
                budget_set_select_q=budget_set_select_q,
            ...
            choices_df = pd.read_sql_query(
                "select * from " + choice_table_name, con=engine
            )
            ...
            for index, row in choices_df.iterrows():
                ...
                if row.option_name not in choice_option_map[row.choice_name]:
                    ...
                    option_name_to_memo_regex_list_map[
                        row.choice_name + " | " + row.option_name
                    ] = row.memo_regexes
            ...
            for choice_name, option_name_list in choice_option_map.items():
                ...
                for option_name in option_name_list:
                    list_of_lists_of_memo_regexes.append(
                        option_name_to_memo_regex_list_map[
                            choice_name + " | " + option_name
                        ].split(";")
                    )
                S.addChoiceToAllForecasts(
                    option_name_list, list_of_lists_of_memo_regexes
                )
            S.writeToDatabase(
                args.database_hostname,
                args.database_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--database_hostname", required=False, help="database hostname", action="store"
    )
    parser.add_argument(
        "--database_name", required=False, help="database name", action="store"
    )
    parser.add_argument(
        "--database_username", required=False, help="database username", action="store"
    )
    parser.add_argument(
        "--database_password", required=False, help="database password", action="store"
    )
    parser.add_argument(
        "--database_port", required=False, help="database port", action="store"
    )
    parser.add_argument(
        "--log_directory",
        default="./log/",
    ...
    parser.add_argument(
        "--id", help="Id of forecastset or forecast to run.", action="store"
    )
    parser.add_argument("--start_date", help="Start date", action="store")
    parser.add_argument("--end_date", help="End date", action="store")
    parser.add_argument(
        "--label",
        required=False,
