...
class ExpenseForecast:
    ...
    def runForecast(self):
        ...
        log_in_color(logger, 'white', 'info', 'Starting Forecast')
    ...
    def writeToJSONFile(self):
        ...
        log_in_color(logger,'green', 'info', 'Writing to ./out/Forecast__' + self.unique_id + '__' + self.start_ts + '.json')
    ...
    def attemptTransaction(self, forecast_df, account_set, memo_set, confirmed_df, proposed_row_df):
        ...
        try:
            ...
            log_in_color(logger, 'red', 'info', account_set.getAccounts().to_string(), self.log_stack_depth)
            log_in_color(logger, 'red', 'info', proposed_row_df.to_string(), self.log_stack_depth)
    ...
    def assessPotentialOptimizations(self, forecast_df, account_set, memo_rule_set, confirmed_df, proposed_df, deferred_df, skipped_df, raise_satisfice_failed_exception):
        ...
        if not raise_satisfice_failed_exception:
            ...
            log_in_color(logger, 'white', 'info', self.start_date_YYYYMMDD + ' -> ' + self.end_date_YYYYMMDD)
            log_in_color(logger, 'white', 'info', 'Priority Indices: ' + str(unique_priority_indices))
        ...
        for priority_index in unique_priority_indices:
            ...
            for date_string_YYYYMMDD in all_days:
                ...
                if not raise_satisfice_failed_exception:
                    ...
                    log_in_color(logger, 'white', 'info', log_string )
                ...
                def initialize_from_excel_file(path_to_excel_file):
                    ...
                    account_milestones_df = pd.read_excel(path_to_excel_file, sheet_name='AccountMilestones')
                    memo_milestones_df = pd.read_excel(path_to_excel_file, sheet_name='MemoMilestones')
                    composite_account_milestones_df = pd.read_excel(path_to_excel_file,
                                                                    sheet_name='CompositeAccountMilestones')
                    composite_memo_milestones_df = pd.read_excel(path_to_excel_file,
                                                                 sheet_name='CompositeMemoMilestones')
                    config_df = pd.read_excel(path_to_excel_file, sheet_name='config')
                    ...
                    for index, row in account_set_df.iterrows():
                        if row.Account_Type.lower() == 'checking':
                            A.createAccount(row.Name, row.Balance, row.Min_Balance, row.Max_Balance, 'checking', None,
                        ...
                        # todo i was tired when I wrote these likely worth a second look
                        if row.Account_Type.lower() == 'curr stmt bal' and expect_curr_bal_acct:
                            A.createAccount(row.Name.split(':')[0], row.Balance, row.Min_Balance, row.Max_Balance,
                                            'credit', billing_start_date, interest_type, apr, interest_cadence,
                                            minimum_payment, previous_statement_balance, None, None)
                            expect_curr_bal_acct = False

                        if row.Account_Type.lower() == 'prev stmt bal' and expect_prev_bal_acct:
                            A.createAccount(row.Name.split(':')[0], current_statement_balance, row.Min_Balance,
                                            row.Max_Balance, 'credit', str(int(row.Billing_Start_Dt)),
                                            row.Interest_Type, row.APR, row.Interest_Cadence, row.Minimum_Payment,
                                            row.Balance, None, None, None)
                            expect_prev_bal_acct = False

                        if row.Account_Type.lower() == 'interest' and expect_interest_acct:
                            A.createAccount(row.Name.split(':')[0], row.Balance + principal_balance, row.Min_Balance,
                                            row.Max_Balance, 'loan', billing_start_date, interest_type, apr,
                                            interest_cadence, minimum_payment, None, principal_balance, row.Balance)
                            expect_interest_acct = False

                        if row.Account_Type.lower() == 'principal balance' and expect_principal_bal_acct:
                            A.createAccount(row.Name.split(':')[0], row.Balance + accrued_interest, row.Min_Balance,
                                            row.Max_Balance, 'loan', str(int(row.Billing_Start_Dt)), row.Interest_Type,
                                            row.APR, row.Interest_Cadence, row.Minimum_Payment, None, row.Balance,
                                            accrued_interest)
                            expect_principal_bal_acct = False

                    B = BudgetSet.BudgetSet([])
                    for index, row in budget_set_df.iterrows():
                        B.addBudgetItem(row.Start_Date, row.End_Date, row.Priority, row.Cadence, row.Amount, row.Memo,
                                        row.Deferrable, row.Partial_Payment_Allowed)
                    ...
                    for index, row in account_milestones_df.iterrows():
                        am__list.append(
                            AccountMilestone.AccountMilestone(row.Milestone_Name, row.Account_Name, row.Min_Balance,
                                                              row.Max_Balance))
                ...
                def initialize_from_json_file(path_to_json):
                    ...
                    for Account__dict in initial_account_set:
                        ...
                        if Account__dict['Account_Type'].lower() == 'checking':
                            ...
                            raise ValueError(
                                'unrecognized account type in ExpenseForecast::initialize_from_json_file: ' + str(
                                    Account__dict['Account_Type']))
                    ...
                    # todo idk what the data structure is here i have to develop the test more
                    for am in milestone_set["account_milestones"]:
                        MS.addAccountMilestone(am['Milestone_Name'], am['Account_Name'], am['Min_Balance'],
                                               am['Max_Balance'])
                    ...
                    for cm in milestone_set["composite_milestones"]:
                        ...
                        for acc_mil in cm['account_milestones']:
                            account_milestones__list.append(
                                AccountMilestone.AccountMilestone(acc_mil['Milestone_Name'], acc_mil['Account_Name'],
                                                                  acc_mil['Min_Balance'], acc_mil['Max_Balance']))

                        memo_milestones__list = []
                        for memo_mil in cm['memo_milestones']:
                            memo_milestones__list.append(
                                MemoMilestone.MemoMilestone(memo_mil['Milestone_Name'], memo_mil['Memo_Regex']))
                ...
                class ExpenseForecast:
                    ...
                    def __str__(self):
                        ...
                        return_string += """Forecast  #""" + str(
                            self.unique_id) + """: """ + self.start_date_YYYYMMDD + """ -> """ + self.end_date_YYYYMMDD + "\n"
                        return_string += " " + (
                                    str(self.initial_account_set.getAccounts().shape[0]) + """ accounts, """ + str(
                                self.initial_budget_set.getBudgetItems().shape[0]) + """  budget items, """ + str(
                                self.initial_memo_rule_set.getMemoRules().shape[0]) + """ memo rules.""").rjust(
                            left_margin_width, ' ') + "\n"
                        return_string += " " + json_file_name + "\n"
                    ...
                    def __init__(self, account_set, budget_set, memo_rule_set, start_date_YYYYMMDD, end_date_YYYYMMDD,
                        ...
                        :param memo_rule_set:
                        """
                        log_in_color(logger, 'green', 'info', 'ExpenseForecast(start_date_YYYYMMDD=' + str(
                            start_date_YYYYMMDD) + ', end_date_YYYYMMDD=' + str(end_date_YYYYMMDD) + ')')
                        ...
                        if datetime.datetime.strptime(str(start_date_YYYYMMDD), '%Y%m%d') >= datetime.datetime.strptime(
                                str(end_date_YYYYMMDD), '%Y%m%d'):
                            raise ValueError(str(self.start_date_YYYYMMDD) + " >= " + str(
                                self.end_date_YYYYMMDD))  # start_date must be before end_date
                        ...
                        distinct_base_account_names__from_acct = pd.DataFrame(
                            pd.DataFrame(accounts_df[['Name']]).apply(lambda x: x[0].split(':')[0],
                                                                      axis=1).drop_duplicates()).rename(
                            columns={0: 'Name'})
                        account_names__from_memo = pd.concat(
                        ...
                        distinct_account_names__from_memo = pd.DataFrame(account_names__from_memo.loc[
                                                                             account_names__from_memo.Name != 'None', 'Name'].drop_duplicates().reset_index(
                            drop=True))

                        try:
                            A = set(distinct_account_names__from_memo.Name).union(set(['']))
                            A = A - set([
                                            'ALL_LOANS'])  # if we have a memo rule for ALL_LOANS, we don't want that to be checked against the list of account names
                        ...
                        if budget_df.shape[0] > 0:
                            # for each budget item memo x priority combo, there is at least 1 memo_regex x priority that matches
                            distinct_memo_priority_combinations__from_budget = budget_df[
                                ['Priority', 'Memo']].drop_duplicates()
                        ...
                        # lb_sel_vec = (datetime.datetime.strptime(self.start_date_YYYYMMDD, '%Y%m%d') <= datetime.datetime.strptime(proposed_df.Date, '%Y%m%d'))
                        lb_sel_vec = [datetime.datetime.strptime(self.start_date_YYYYMMDD,
                                                                 '%Y%m%d') <= datetime.datetime.strptime(d, '%Y%m%d')
                                      for d in proposed_df.Date]
                        # rb_sel_vec = (datetime.datetime.strptime(proposed_df.Date, '%Y%m%d') <= datetime.datetime.strptime(str(end_date_YYYYMMDD), '%Y%m%d'))
                        rb_sel_vec = [datetime.datetime.strptime(d, '%Y%m%d') <= datetime.datetime.strptime(
                            self.end_date_YYYYMMDD, '%Y%m%d') for d in proposed_df.Date]
                        ...
                        self.unique_id = str(hash(int(account_hash, 16) + int(budget_hash, 16) + int(memo_hash,
                                                                                                     16) + start_date_hash + end_date_hash) % 100000).rjust(
                            6, '0')
                        #
                        # print("unique_id:"+str(self.unique_id))
                        # print("")
                    ...
                    def appendSummaryLines(self):
                        ...
                        cc_acct_sel_vec = (account_info.Account_Type == 'prev stmt bal') | (
                                    account_info.Account_Type == 'curr stmt bal')
                    ...
                    def writeToJSONFile(self):
                        ...
                        log_in_color(logger, 'green', 'info',
                                     'Writing to ./out/Forecast__' + self.unique_id + '__' + self.start_ts + '.json')
                        # self.forecast_df.to_csv('./Forecast__' + run_ts + '.json')
                    ...
                    def addANewDayToTheForecast(self, forecast_df, date_YYYYMMDD):
                        dates_as_datetime_dtype = [datetime.datetime.strptime(d, '%Y%m%d') for d in forecast_df.Date]
                        prev_date_as_datetime_dtype = (
                                    datetime.datetime.strptime(date_YYYYMMDD, '%Y%m%d') - datetime.timedelta(days=1))
                        sel_vec = [d == prev_date_as_datetime_dtype for d in dates_as_datetime_dtype]
                        new_row_df = copy.deepcopy(forecast_df.loc[sel_vec])
                        new_row_df.Date = date_YYYYMMDD
                        new_row_df.Memo = ''
                    ...
                    def checkIfTxnIsIncome(self, confirmed_row):
                        ...
                        try:
                            ...
                            log_in_color(logger, 'yellow', 'debug',
                                         'transaction flagged as income: ' + str(m_income.group(0)), 3)
                        except Exception as e:
                            income_flag = False

                        return income_flag

                    def updateBalancesAndMemo(self, forecast_df, account_set, confirmed_row, memo_rule_row,
                                              date_YYYYMMDD):
                        # log_in_color(logger, 'green', ''info'', 'ENTER updateBalancesAndMemo()',self.log_stack_depth)
                        ...
                        if memo_rule_row.Account_To != 'ALL_LOANS':
                            forecast_df.loc[
                                row_sel_vec, forecast_df.columns == 'Memo'] += confirmed_row.Memo + ' ($' + str(
                                confirmed_row.Amount) + ') ; '
                            log_in_color(logger, 'green', 'info',
                                         confirmed_row.Memo + ' ($' + str(confirmed_row.Amount) + ') ; ',
                                         self.log_stack_depth)
                            ...
                            if current_balance != relevant_balance:
                                ...
                                if memo_rule_row.Account_To == 'ALL_LOANS' and account_row.Name != memo_rule_row.Account_From:
                                    forecast_df.loc[row_sel_vec, forecast_df.columns == 'Memo'] += str(
                                        ...
                                        current_balance - relevant_balance) + ') ; '
                                    log_in_color(logger, 'green', 'info',
                                                 str(account_row.Name) + ' additional loan payment ($' + str(
                                                     current_balance - relevant_balance) + ') ; ', self.log_stack_depth)
                    ...
                    def attemptTransaction(self, forecast_df, account_set, memo_set, confirmed_df, proposed_row_df):
                        log_in_color(logger, 'green', 'info',
                                     'ENTER attemptTransaction( C:' + str(confirmed_df.shape[0]) + ' P:' + str(
                                         proposed_row_df.Memo) + ')', self.log_stack_depth)
                        self.log_stack_depth += 1
                        try:
                            single_proposed_transaction_df = pd.DataFrame(copy.deepcopy(proposed_row_df)).T
                            ...
                            empty_df = pd.DataFrame(
                                {'Date': [], 'Priority': [], 'Amount': [], 'Memo': [], 'Deferrable': [],
                                 'Partial_Payment_Allowed': []})
                            ...
                            hypothetical_future_state_of_forecast = \
                                self.computeOptimalForecast(start_date_YYYYMMDD=self.start_date_YYYYMMDD,
                                                            ...
                                                            skipped_df=empty_df,
                                                            account_set=copy.deepcopy(
                                                                self.sync_account_set_w_forecast_day(account_set,
                                                                                                     forecast_df,
                                                                                                     self.start_date_YYYYMMDD)),
                                                            memo_rule_set=memo_set)[0]
                            ...
                            self.log_stack_depth -= 5  # several decrements were skipped over by the exception
                            log_in_color(logger, 'red', 'info', str(e), self.log_stack_depth)
                            log_in_color(logger, 'red', 'info', account_set.getAccounts().to_string(),
                                         self.log_stack_depth)
                            log_in_color(logger, 'red', 'info', proposed_row_df.to_string(), self.log_stack_depth)
                            log_in_color(logger, 'red', 'info',
                                         'EXIT attemptTransaction(' + str(proposed_row_df.Memo) + ')' + ' (FAIL)',
                                         self.log_stack_depth)
                            if re.search('.*Account boundaries were violated.*',
                                         str(e.args)) is None:  # this is the only exception where we don't want to stop immediately
                                raise e

                    def processConfirmedTransactions(self, forecast_df, relevant_confirmed_df, memo_set, account_set,
                                                     date_YYYYMMDD):
                        log_in_color(logger, 'green', 'info', 'ENTER processConfirmedTransactions( C:' + str(
                            relevant_confirmed_df.shape[0]) + ' )', self.log_stack_depth)
                        self.log_stack_depth += 1

                        for confirmed_index, confirmed_row in relevant_confirmed_df.iterrows():
                            relevant_memo_rule_set = memo_set.findMatchingMemoRule(confirmed_row.Memo,
                                                                                   confirmed_row.Priority)
                            memo_rule_row = relevant_memo_rule_set.getMemoRules().loc[0, :]
                            ...
                            forecast_df = self.updateBalancesAndMemo(forecast_df, account_set, confirmed_row,
                                                                     memo_rule_row,
                                                                     date_YYYYMMDD)

                        self.log_stack_depth -= 1
                        log_in_color(logger, 'green', 'info', 'EXIT processConfirmedTransactions()',
                                     self.log_stack_depth)
                        return forecast_df

                    def processProposedTransactions(self, account_set, forecast_df, date_YYYYMMDD, memo_set,
                                                    confirmed_df, relevant_proposed_df, priority_level):
                        log_in_color(logger, 'green', 'info', 'ENTER processProposedTransactions( P:' + str(
                            relevant_proposed_df.shape[0]) + ' )', self.log_stack_depth)
                        self.log_stack_depth += 1
                        ...
                        if relevant_proposed_df.shape[0] == 0:
                            self.log_stack_depth -= 1
                            log_in_color(logger, 'green', 'info', 'EXIT processProposedTransactions()',
                                         self.log_stack_depth)
                            return forecast_df, new_confirmed_df, new_deferred_df, new_skipped_df

                        for proposed_item_index, proposed_row_df in relevant_proposed_df.iterrows():

                            relevant_memo_rule_set = memo_set.findMatchingMemoRule(proposed_row_df.Memo,
                                                                                   proposed_row_df.Priority)
                            memo_rule_row = relevant_memo_rule_set.getMemoRules().loc[0, :]

                            # False if not permitted, updated forecast if it is permitted
                            result_of_attempt = self.attemptTransaction(forecast_df, account_set, memo_set,
                                                                        confirmed_df,
                                                                        proposed_row_df)
                            log_in_color(logger, 'green', 'info', 'result_of_attempt:', self.log_stack_depth)
                            log_in_color(logger, 'green', 'info', str(result_of_attempt), self.log_stack_depth)
                            ...
                            if not transaction_is_permitted and proposed_row_df.Partial_Payment_Allowed:
                                ...
                                try:
                                    ...
                                    min_fut_avl_bals = self.getMinimumFutureAvailableBalances(account_set, forecast_df,
                                                                                              date_YYYYMMDD)
                                    ...
                                    not_yet_validated_confirmed_df = pd.concat(
                                        [confirmed_df, pd.DataFrame(proposed_row_df).T])
                                    ...
                                    hypothetical_future_state_of_forecast = \
                                        self.computeOptimalForecast(start_date_YYYYMMDD=self.start_date_YYYYMMDD,
                                                                    ...
                                                                    skipped_df=empty_df,
                                                                    account_set=copy.deepcopy(
                                                                        self.sync_account_set_w_forecast_day(
                            ...
                            if not transaction_is_permitted and proposed_row_df.Deferrable:
                                ...
                                proposed_row_df.Date = (datetime.datetime.strptime(proposed_row_df.Date,
                                ...
                                if priority_level > 1:
                                    account_set = self.sync_account_set_w_forecast_day(account_set, forecast_df,
                                                                                       date_YYYYMMDD)
                                ...
                                forecast_rows_to_keep_df = forecast_df[
                                    [datetime.datetime.strptime(d, '%Y%m%d') < datetime.datetime.strptime(date_YYYYMMDD,
                                                                                                          '%Y%m%d') for
                                     d
                                     in forecast_df.Date]]

                                new_forecast_rows_df = forecast_with_accurately_updated_future_rows[
                                    [datetime.datetime.strptime(d, '%Y%m%d') >= datetime.datetime.strptime(
                                        date_YYYYMMDD, '%Y%m%d') for
                                     d in forecast_with_accurately_updated_future_rows.Date]]
                                forecast_df = pd.concat([forecast_rows_to_keep_df, new_forecast_rows_df])
                                assert forecast_df.shape[0] == forecast_df.drop_duplicates().shape[0]
                                forecast_df.reset_index(drop=True, inplace=True)

                                row_sel_vec = [x for x in (
                                            forecast_df.Date == datetime.datetime.strptime(date_YYYYMMDD, '%Y%m%d'))]
                                col_sel_vec = (forecast_df.columns == "Memo")
                                ...
                                raise ValueError("""This is an edge case that should not be possible
                                        transaction_is_permitted...............:""" + str(transaction_is_permitted) + """
                                        budget_item_row.Deferrable.............:""" + str(proposed_row_df.Deferrable) + """
                                        budget_item_row.Partial_Payment_Allowed:""" + str(
                                    proposed_row_df.Partial_Payment_Allowed) + """
                                        """)

                        self.log_stack_depth -= 1
                        log_in_color(logger, 'green', 'info', 'EXIT processProposedTransactions() C:' + str(
                            new_confirmed_df.shape[0]) + ' D:' + str(new_deferred_df.shape[0]) + ' S:' + str(
                            new_skipped_df.shape[0]), self.log_stack_depth)
                        return forecast_df, new_confirmed_df, new_deferred_df, new_skipped_df
                    ...
                    # account_set, forecast_df, date_YYYYMMDD, memo_set,              ,    relevant_deferred_df,             priority_level, allow_partial_payments, allow_skip_and_defer
                    def processDeferredTransactions(self, account_set, forecast_df, date_YYYYMMDD, memo_set,
                                                    relevant_deferred_df, priority_level, confirmed_df):
                        log_in_color(logger, 'green', 'info', 'ENTER processDeferredTransactions( D:' + str(
                            relevant_deferred_df.shape[0]) + ' )', self.log_stack_depth)
                        self.log_stack_depth += 1
                        ...
                        if relevant_deferred_df.shape[0] == 0:
                            self.log_stack_depth -= 1
                            log_in_color(logger, 'green', 'info', 'EXIT processDeferredTransactions()',
                                         self.log_stack_depth)
                            return forecast_df, new_confirmed_df, new_deferred_df
                        ...
                        for deferred_item_index, deferred_row_df in relevant_deferred_df.iterrows():
                            ...
                            relevant_memo_rule_set = memo_set.findMatchingMemoRule(deferred_row_df.Memo,
                                                                                   deferred_row_df.Priority)
                            memo_rule_row = relevant_memo_rule_set.getMemoRules().loc[0, :]
                            ...
                            if not transaction_is_permitted and deferred_row_df.Deferrable:
                                deferred_row_df.Date = (
                                        datetime.datetime.strptime(deferred_row_df.Date,
                                                                   '%Y%m%d') + datetime.timedelta(days=1)).strftime(
                                    '%Y%m%d')
                                # _deferred_df = relevant_deferred_df[    ~relevant_deferred_df.index.isin(deferred_row_df.index)]
                                ...
                                raise ValueError("""This is an edge case that should not be possible
                                        ...
                                        budget_item_row.Deferrable.............:""" + str(deferred_row_df.Deferrable) + """
                                        budget_item_row.Partial_Payment_Allowed:""" + str(
                                    deferred_row_df.Partial_Payment_Allowed) + """
                                        """)
                    ...
                    def executeTransactionsForDay(self, account_set, forecast_df, date_YYYYMMDD, memo_set, confirmed_df,
                                                  proposed_df, deferred_df, skipped_df, priority_level):
                        """
                        ...
                        row_count_string = ' C0:' + str(C0) + '  P0:' + str(P0) + '  D0:' + str(D0) + '  S0:' + str(
                            S0) + '  T0:' + str(T0)
                        log_in_color(logger, 'green', 'info', 'ENTER executeTransactionsForDay(priority_level=' + str(
                            priority_level) + ',date=' + str(date_YYYYMMDD) + ') ' + str(row_count_string) + str(
                            bal_string), self.log_stack_depth)

                        self.log_stack_depth += 1
                        isP1 = (priority_level == 1)

                        relevant_proposed_df = copy.deepcopy(
                            proposed_df[(proposed_df.Priority == priority_level) & (proposed_df.Date == date_YYYYMMDD)])
                        ...
                        relevant_deferred_df = copy.deepcopy(
                            deferred_df[(deferred_df.Priority <= priority_level) & (deferred_df.Date == date_YYYYMMDD)])
                        ...
                        date_sel_vec = [(d == date_YYYYMMDD) for d in forecast_df.Date]
                        noMatchingDayInForecast = forecast_df.loc[date_sel_vec].empty
                        notPastEndOfForecast = datetime.datetime.strptime(date_YYYYMMDD,
                        ...
                        log_in_color(logger, 'green', 'debug', 'eTFD :: before processConfirmed', self.log_stack_depth)
                        forecast_df = self.processConfirmedTransactions(forecast_df, relevant_confirmed_df, memo_set,
                                                                        account_set, date_YYYYMMDD)
                        log_in_color(logger, 'green', 'debug', 'eTFD :: after processConfirmed', self.log_stack_depth)
                        ...
                        relevant_deferred_before_processing = pd.DataFrame(relevant_deferred_df,
                                                                           copy=True)  # we need this to remove old txns if they stay deferred

                        log_in_color(logger, 'green', 'debug', 'eTFD :: before processDeferred', self.log_stack_depth)
                        forecast_df, new_confirmed_df, new_deferred_df = self.processDeferredTransactions(account_set,
                                                                                                          forecast_df,
                                                                                                          date_YYYYMMDD,
                                                                                                          memo_set,
                                                                                                          pd.DataFrame(
                        ...
                        # deferred_df = deferred_df - relevant + new. index won't be the same as OG
                        # this is the inverse of how we selected the relevant rows
                        not_relevant_deferred_df = pd.DataFrame(
                            deferred_df[(deferred_df.Priority > priority_level) | (deferred_df.Date != date_YYYYMMDD)],
                            copy=True)
                        ...
                        self.log_stack_depth -= 1
                        log_in_color(logger, 'green', 'info', 'EXIT executeTransactionsForDay(priority_level=' + str(
                            priority_level) + ',date=' + str(date_YYYYMMDD) + ') ' + str(row_count_string) + str(
                            bal_string), self.log_stack_depth)
                        return [forecast_df, confirmed_df, deferred_df, skipped_df]
                    ...
                    def calculateInterestAccrualsForDay(self, account_set, current_forecast_row_df):
                        ...
                        log_in_color(logger, 'cyan', 'debug',
                                     'ENTER calculateInterestAccrualsForDay() ' + current_forecast_row_df.Date.iat[
                                         0] + bal_string, self.log_stack_depth)
                        self.log_stack_depth += 1
                        # This method will transfer balances from current statement to previous statement for savings and credit accounts
                        ...
                        for account_index, account_row in account_set.getAccounts().iterrows():
                            if account_row.Interest_Cadence == 'None' or account_row.Interest_Cadence is None or account_row.Interest_Cadence == '':  # ithink this may be refactored. i think this will explode if interest_cadence is None
                                continue
                            num_days = (datetime.datetime.strptime(current_date, '%Y%m%d') - datetime.datetime.strptime(
                                account_row.Billing_Start_Dt, '%Y%m%d')).days
                            dseq = generate_date_sequence(start_date_YYYYMMDD=account_row.Billing_Start_Dt,
                                                          num_days=num_days, cadence=account_row.Interest_Cadence)
                            ...
                            if current_date in dseq:
                                ...
                                if account_row.Interest_Type.lower() == 'compound' and account_row.Interest_Cadence.lower() == 'yearly':
                                    ...
                                    account_set.accounts[
                                        account_index + 1].balance += accrued_interest  # this is the interest account
                                    account_set.accounts[account_index + 1].balance = round(
                                        account_set.accounts[account_index + 1].balance, 2)
                                    if abs(account_set.accounts[account_index + 1].balance) < 0.01:
                                        account_set.accounts[account_index + 1].balance = 0
                                    # account_set.accounts[account_index + 1].balance = account_set.accounts[account_index + 1].balance
                        ...
                        self.log_stack_depth -= 1
                        log_in_color(logger, 'cyan', 'debug',
                                     'EXIT  calculateInterestAccrualsForDay() ' + current_forecast_row_df.Date.iat[
                                         0] + bal_string, self.log_stack_depth)
                        # logger.debug('self.log_stack_depth -= 1')
                        return current_forecast_row_df  # this runs when there are no interest bearing accounts in the simulation at all
                    ...
                    def executeMinimumPayments(self, account_set, current_forecast_row_df):
                        ...
                        # logger.debug('self.log_stack_depth += 1')
                        log_in_color(logger, 'cyan', 'debug', 'ENTER executeMinimumPayments() ' + bal_string,
                                     self.log_stack_depth)
                        self.log_stack_depth += 1
                        ...
                        for account_index, account_row in account_set.getAccounts().iterrows():
                            ...
                            # billing_days = set(generate_date_sequence(account_row.Billing_Start_Dt.strftime('%Y%m%d'), num_days, account_row.Interest_Cadence))
                            billing_days = set(
                                generate_date_sequence(account_row.Billing_Start_Dt, num_days, 'monthly'))
                            ...
                            if current_forecast_row_df.Date.iloc[0] in billing_days:
                                ...
                                if account_row.Account_Type == 'prev stmt bal':  # cc min payment
                                    ...
                                    payment_toward_prev = min(minimum_payment_amount, account_row.Balance)
                                    payment_toward_curr = min(
                                        account_set.getAccounts().loc[account_index - 1, :].Balance,
                                        minimum_payment_amount - payment_toward_prev)
                                    surplus_payment = minimum_payment_amount - (
                                                payment_toward_prev + payment_toward_curr)
                                    ...
                                    if (payment_toward_prev + payment_toward_curr) > 0:
                                        account_set.executeTransaction(Account_From='Checking',
                                                                       ...
                                                                       Amount=(
                                                                                   payment_toward_prev + payment_toward_curr))
                                        current_forecast_row_df.Memo += account_row.Name.split(':')[
                                                                            0] + ' cc min payment ($' + str(
                                            minimum_payment_amount) + ') ; '

                                elif account_row.Account_Type == 'principal balance':  # loan min payment

                                    minimum_payment_amount = account_set.getAccounts().loc[account_index,
                                                             :].Minimum_Payment
                                    current_debt_balance = account_set.getBalances()[account_row.Name.split(':')[0]]
                                    loan_payment_amount = min(current_debt_balance, minimum_payment_amount)
                        ...
                        # log_in_color(logger,'green', 'debug', current_forecast_row_df.to_string(), self.log_stack_depth)
                        self.log_stack_depth -= 1
                        log_in_color(logger, 'cyan', 'debug', 'EXIT  executeMinimumPayments() ' + bal_string,
                                     self.log_stack_depth)
                        # logger.debug('self.log_stack_depth -= 1')
                    ...
                    def getMinimumFutureAvailableBalances(self, account_set, forecast_df, date_YYYYMMDD):
                        ...
                        # logger.debug('self.log_stack_depth += 1')
                        log_in_color(logger, 'green', 'INFO',
                                     'ENTER getMinimumFutureAvailableBalances(date=' + str(date_YYYYMMDD) + ')',
                                     self.log_stack_depth)
                        log_in_color(logger, 'green', 'INFO', forecast_df.to_string(), self.log_stack_depth)
                        self.log_stack_depth += 1
                        ...
                        for account_index, account_row in A.iterrows():
                            ...
                            if account_row.Account_Type.lower() == 'checking':
                                ...
                                available_credit = current_and_future_forecast_df[prev_name] + \
                                                   current_and_future_forecast_df[curr_name]
                                future_available_balances[aname] = A[A.Name == prev_name].Max_Balance.iloc[0] - min(
                                    available_credit)
                        ...
                        self.log_stack_depth -= 1
                        log_in_color(logger, 'green', 'info',
                                     'EXIT getMinimumFutureAvailableBalances(date=' + str(date_YYYYMMDD) + ')',
                                     self.log_stack_depth)
                        # logger.debug('self.log_stack_depth -= 1')
                        return future_available_balances
                    ...
                    def sync_account_set_w_forecast_day(self, account_set, forecast_df, date_YYYYMMDD):
                        ...
                        for index, row in account_set.getAccounts().iterrows():
                            bal_string += '$' + str(row.Balance) + ' '
                        log_in_color(logger, 'cyan', 'debug',
                                     'ENTER sync_account_set_w_forecast_day(date_YYYYMMDD=' + str(
                                         date_YYYYMMDD) + ')' + bal_string, self.log_stack_depth)
                        # log_in_color(logger,'green','debug','Initial account_set:',self.log_stack_depth)
                        # log_in_color(logger,'green', 'debug', account_set.getAccounts().to_string(), self.log_stack_depth)
                        ...
                        for index, row in account_set.getAccounts().iterrows():
                            bal_string += '$' + str(row.Balance) + ' '
                        log_in_color(logger, 'cyan', 'debug',
                                     'EXIT sync_account_set_w_forecast_day(date_YYYYMMDD=' + str(
                                         date_YYYYMMDD) + ')' + bal_string, self.log_stack_depth)
                        return account_set
                    ...
                    # propagate into the future and raise exception if account boundaries are violated
                    def propagateTransactionsIntoTheFuture(self, account_set_before_p2_plus_txn, forecast_df,
                    ...
                    def updateProposedTransactionsBasedOnOtherSets(self, confirmed_df, proposed_df, deferred_df,
                                                                   skipped_df):
                        # update remaining_unproposed_transactions_df based on modifications to other sets made during the last loop
                        C = 'C:' + str(confirmed_df.shape[0])
                        P = 'P:' + str(proposed_df.shape[0])
                        ...
                        log_in_color(logger, 'cyan', 'info',
                                     'ENTER updateProposedTransactionsBasedOnOtherSets( ' + C + ' ' + P + ' ' + D + ' ' + S + ' )',
                                     self.log_stack_depth)
                        ...
                        self.log_stack_depth -= 1
                        log_in_color(logger, 'cyan', 'info', 'EXIT updateProposedTransactionsBasedOnOtherSets',
                                     self.log_stack_depth)
                        return remaining_unproposed_transactions_df

                    def overwriteOGSatisficeInterestWhenAdditionalLoanPayment(self, forecast_df, date_string_YYYYMMDD,
                                                                              account_set):
                        bal_string = ' '
                        for index, row in account_set.getAccounts().iterrows():
                            bal_string += '$' + str(row.Balance) + ' '
                        log_in_color(logger, 'cyan', 'debug',
                                     'ENTER overwriteOGSatisficeInterestWhenAdditionalLoanPayment()' + bal_string,
                                     self.log_stack_depth)

                        # logic to update interest accruals if principal balance has been reduced
                        yesterday_date_string = (
                                    datetime.datetime.strptime(date_string_YYYYMMDD, '%Y%m%d') - datetime.timedelta(
                                days=1)).strftime('%Y%m%d')
                        yesterdays_values = self.sync_account_set_w_forecast_day(account_set, forecast_df,
                                                                                 yesterday_date_string)
                        forecast_row_w_new_interest_values = self.calculateInterestAccrualsForDay(yesterdays_values,
                                                                                                  forecast_df[
                                                                                                      forecast_df.Date == yesterday_date_string])  # returns only a forecast row
                        forecast_row_w_new_interest_values.Date = date_string_YYYYMMDD
                        forecast_row_w_new_interest_values.Memo = ''
                        for i in range(0, forecast_row_w_new_interest_values.shape[1]):
                            if not ': Interest' in forecast_row_w_new_interest_values.columns[i]:
                            ...
                            if forecast_df.loc[
                                forecast_df.Date == date_string_YYYYMMDD, forecast_row_w_new_interest_values.columns[
                                    i]].iat[0] < \
                                    forecast_row_w_new_interest_values.iloc[0, i]:
                                pass
                            else:
                                forecast_df.loc[forecast_df.Date == date_string_YYYYMMDD,
                                                forecast_row_w_new_interest_values.columns[i]] = \
                                    forecast_row_w_new_interest_values.iloc[0, i]
                                log_in_color(logger, 'cyan', 'debug', 'OVERWRITING INTEREST VALUE' + bal_string,
                                             self.log_stack_depth)
                        ...
                        for index, row in account_set.getAccounts().iterrows():
                            bal_string += '$' + str(row.Balance) + ' '
                        log_in_color(logger, 'cyan', 'debug',
                                     'EXIT overwriteOGSatisficeInterestWhenAdditionalLoanPayment()' + bal_string,
                                     self.log_stack_depth)
                        return forecast_df

                    def assessPotentialOptimizations(self, forecast_df, account_set, memo_rule_set, confirmed_df,
                                                     proposed_df, deferred_df, skipped_df,
                        ...
                        S = 'S:' + str(skipped_df.shape[0])
                        log_in_color(logger, 'magenta', 'info',
                                     'ENTER assessPotentialOptimizations( ' + F + ' ' + C + ' ' + P + ' ' + D + ' ' + S + ' )',
                                     self.log_stack_depth)
                        self.log_stack_depth += 1
                        all_days = forecast_df.Date  # todo havent tested this, but forecast_df has been satisficed so it has all the dates
                        ...
                        if not raise_satisfice_failed_exception:
                            ...
                            log_in_color(logger, 'white', 'info',
                                         self.start_date_YYYYMMDD + ' -> ' + self.end_date_YYYYMMDD)
                            log_in_color(logger, 'white', 'info', 'Priority Indices: ' + str(unique_priority_indices))
                            last_iteration_ts = datetime.datetime.now()
                        ...
                        for priority_index in unique_priority_indices:
                            ...
                            for date_string_YYYYMMDD in all_days:
                                ...
                                if not raise_satisfice_failed_exception:
                                    iteration_time_elapsed = datetime.datetime.now() - last_iteration_ts
                                    last_iteration_ts = datetime.datetime.now()
                                    log_string = str(priority_index) + ' ' + datetime.datetime.strptime(
                                        date_string_YYYYMMDD, '%Y%m%d').strftime('%Y-%m-%d')
                                    log_string += '     ' + str(iteration_time_elapsed)
                                    log_in_color(logger, 'white', 'info', log_string)

                                log_in_color(logger, 'magenta', 'info',
                                             'p' + str(priority_index) + ' ' + str(date_string_YYYYMMDD),
                                             self.log_stack_depth)

                                remaining_unproposed_transactions_df = self.updateProposedTransactionsBasedOnOtherSets(
                                ...
                                # a propogateChanges method in the previous loop iteration
                                # feeling more confident now... this is making sense
                                forecast_df = self.overwriteOGSatisficeInterestWhenAdditionalLoanPayment(forecast_df,
                                                                                                         date_string_YYYYMMDD,
                                                                                                         account_set)
                                ...
                                if raise_satisfice_failed_exception:
                                    ...
                                    #
                                    forecast_df = self.propagateTransactionsIntoTheFuture(
                                        account_set_before_p2_plus_txn, forecast_df, date_string_YYYYMMDD)

                        self.log_stack_depth -= 1
                        log_in_color(logger, 'magenta', 'info', 'EXIT assessPotentialOptimizations() C:' + str(
                            confirmed_df.shape[0]) + ' D:' + str(deferred_df.shape[0]) + ' S:' + str(
                            skipped_df.shape[0]), self.log_stack_depth)
                        return forecast_df, skipped_df, confirmed_df, deferred_df
                    ...
                    def cleanUpAfterFailedSatisfice(self, confirmed_df, proposed_df, deferred_df, skipped_df):
                        ...
                        not_confirmed_sel_vec = [(datetime.datetime.strptime(d, '%Y%m%d') > datetime.datetime.strptime(
                            self.end_date_YYYYMMDD, '%Y%m%d')) for d in
                                                 confirmed_df.Date]  # this is using an end date that has been moved forward, so it is > not >=
                        ...
                        skipped_df = pd.concat([skipped_df, not_confirmed_df, new_deferred_df,
                                                deferred_df])  # todo I added deferred_df without testing if that was correct

                        # if it was confirmed before the date of failure, it stays confirmed
                        confirmed_sel_vec = [(datetime.datetime.strptime(d, '%Y%m%d') <= datetime.datetime.strptime(
                            self.end_date_YYYYMMDD, '%Y%m%d')) for d in confirmed_df.Date]
                    ...
                    def satisfice(self, list_of_date_strings, confirmed_df, account_set, memo_rule_set, forecast_df,
                        ...
                        for d in all_days:
                            if d == self.start_date_YYYYMMDD:
                                if not raise_satisfice_failed_exception:
                                    log_in_color(logger, 'white', 'info', 'Starting Satisfice.')
                                    log_in_color(logger, 'white', 'info',
                                                 self.start_date_YYYYMMDD + ' -> ' + self.end_date_YYYYMMDD)
                                    log_in_color(logger, 'white', 'info', 'p Date           iteration time elapsed')
                            ...
                            try:
                                ...
                                if not raise_satisfice_failed_exception:
                                    ...
                                    last_iteration = datetime.datetime.now()
                                    log_string = str(1) + ' ' + datetime.datetime.strptime(d, '%Y%m%d').strftime(
                                        '%Y-%m-%d')
                                ...
                                forecast_df, confirmed_df, deferred_df, skipped_df = \
                                    self.executeTransactionsForDay(account_set=account_set,
                                                                   ...
                                                                   memo_set=memo_rule_set,
                                                                   confirmed_df=confirmed_df,
                                                                   proposed_df=confirmed_df.head(0),
                                                                   # no proposed txns in satisfice
                                                                   deferred_df=confirmed_df.head(0),
                                                                   # no deferred txns in satisfice
                                                                   skipped_df=confirmed_df.head(0),
                                                                   # no skipped txns in satisfice
                                                                   priority_level=1)
                                ...
                                post_min_payments_row = self.executeMinimumPayments(account_set,
                    ...
                    def computeOptimalForecast(self, start_date_YYYYMMDD, end_date_YYYYMMDD, confirmed_df, proposed_df,
                                               deferred_df, skipped_df, account_set, memo_rule_set,
                                               raise_satisfice_failed_exception=True):
                        ...
                        satisfice_success = self.satisfice(all_days, confirmed_df, account_set, memo_rule_set,
                                                           forecast_df, raise_satisfice_failed_exception)
                        ...
                        if satisfice_success is not None:
                            ...
                            forecast_df, skipped_df, confirmed_df, deferred_df = self.assessPotentialOptimizations(
                                forecast_df, account_set, memo_rule_set, confirmed_df, proposed_df, deferred_df,
                                skipped_df, raise_satisfice_failed_exception)
                        else:
                            if not raise_satisfice_failed_exception:
                                log_in_color(logger, 'white', 'info', 'Satisfice failed.')

                            confirmed_df, deferred_df, skipped_df = self.cleanUpAfterFailedSatisfice(confirmed_df,
                                                                                                     proposed_df,
                                                                                                     deferred_df,
                                                                                                     skipped_df)
                        ...
                        self.log_stack_depth -= 1
                        log_in_color(logger, 'cyan', 'info',
                                     'EXIT computeOptimalForecast() C:' + str(confirmed_df.shape[0]) + ' D:' + str(
                                         deferred_df.shape[0]) + ' S:' + str(skipped_df.shape[0]), self.log_stack_depth)
                        return [forecast_df, skipped_df, confirmed_df, deferred_df]
                    ...
                    def evaluateAccountMilestone(self, account_name, min_balance, max_balance):
                        log_in_color(logger, 'yellow', 'debug',
                                     'ENTER evaluateAccountMilestone(' + str(account_name) + ',' + str(
                                         min_balance) + ',' + str(max_balance) + ')')
                        account_info = self.initial_account_set.getAccounts()
                        ...
                        if relevant_account_info_rows_df.shape[0] == 1:  # case for checking and savings
                            col_sel_vec = self.forecast_df.columns == relevant_account_info_rows_df.head(1)['Name'].iat[
                                0]
                            col_sel_vec[0] = True
                        ...
                        if not ((min_balance <= last_value) & (last_value <= max_balance)):
                            log_in_color(logger, 'yellow', 'debug',
                                         'EXIT evaluateAccountMilestone(' + str(account_name) + ',' + str(
                                             min_balance) + ',' + str(max_balance) + ')')
                            return None
                    ...
                    def evaluateCompositeMilestone(self, list_of_account_milestones, list_of_memo_milestones):
                        ...
                        for i in range(0, num_of_acct_milestones):
                            ...
                            am_result = self.evaluateAccountMilestone(account_milestone.account_name,
                    ...
                    def to_json(self):
                        ...
                        normalized_forecast_df_JSON_string = self.forecast_df.to_json(orient='records',
                                                                                      date_format='iso')  # .replace('\'','"')
                        normalized_skipped_df_JSON_string = self.skipped_df.to_json(orient='records',
                                                                                    date_format='iso')  # .replace('\'','"')
                        normalized_confirmed_df_JSON_string = self.confirmed_df.to_json(orient='records',
                                                                                        date_format='iso')  # .replace('\'','"')
                        normalized_deferred_df_JSON_string = self.deferred_df.to_json(orient='records',
                                                                                      date_format='iso')  # .replace('\'','"')
                    ...
                    def compute_forecast_difference(self, forecast_df, forecast2_df, label='forecast_difference',
                                                    make_plots=False, plot_directory='.', return_type='dataframe',
                                                    require_matching_columns=False,
                                                    require_matching_date_range=False, append_expected_values=False,
                                                    diffs_only=False):

                        forecast_df['Date'] = forecast_df.Date.apply(lambda x: datetime.datetime.strptime(x, '%Y%m%d'),
                                                                     0)
                    ...
                    def to_excel(self, path):
                        ...
                        with pd.ExcelWriter(path, engine='openpyxl') as writer:
                            ...
                            composite_account_milestones_df.to_excel(writer, sheet_name='CompositeAccountMilestones',
                                                                     index=False)
                            composite_memo_milestones_df.to_excel(writer, sheet_name='CompositeMemoMilestones',
                                                                  index=False)
                            ...
                            if hasattr(self, 'forecast_df'):
                                ...
                                self.getMilestoneResultsDF().to_excel(writer, sheet_name='MilestoneResults',
                                                                      index=False)
                    ...
                    def getConfigDF(self):
                        return pd.DataFrame({'Start_Date_YYYYMMDD': [self.start_date_YYYYMMDD],
                                             'End_Date_YYYYMMDD': [self.end_date_YYYYMMDD]})
